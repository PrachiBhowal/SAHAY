...
export default function AlertsPanel({ patientId }) {
  ...
  async function resolveAlert(alertId) {
    try {
      const updated = await api.resolveAlert(patientId, alertId);
          ...
          {filtered.map((alert) => {
            ...
            return (
              ...
              >
                ...
                {!alert.resolved && (
                  <button type="button" style={styles.resolveButton} onClick={() => resolveAlert(alert.id)}>
                    Resolve
                  </button>
                )}
              </div>
...
const styles = {
  ...
  resolveButton: { border: "1px solid #B9C7AE", borderRadius: 6, background: "#fff", color: "#4E6142", padding: "5px 8px", cursor: "pointer", flexShrink: 0 },
};