      ...
      <div style={styles.header}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <div style={styles.lastActiveBadge}>
            {lastActive ? `Last active: ${formatLastActive(lastActive)}` : formatLastActive(lastActive)}
          </div>
          <a
            href={`${PATIENT_APP_URL}/?patientId=${patient.id}`}
            target="_blank"
            rel="noopener noreferrer"
            style={styles.openPatientModeBtn}
            title={`Open Patient Mode as ${patient.name}`}
          >
            🎮 Open Patient Mode ↗
          </a>
        </div>
