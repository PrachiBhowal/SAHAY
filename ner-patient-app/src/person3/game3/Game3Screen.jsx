...
import { hydrate, getCurrentTier } from "../../lib/difficultyEngine.js";
import DailyRoutineRecall from "./DailyRoutineRecall";

export default function Game3Screen({ onBack, onComfortReady }) {
  ...
  useEffect(() => {
    ...
    getPatientData()
      .then((patientData) => {
        if (isActive) {
          hydrate(patientData.id, patientData.difficulty_tiers);
          setDifficultyTier(getCurrentTier(patientData.id, "recall"));
  ...
  return (
    <main className="game3-screen">
      ...
      <DailyRoutineRecall patient={patient} difficultyTier={difficultyTier} onTierChange={setDifficultyTier} onComfortReady={onComfortReady} />
    </main>
  );
}